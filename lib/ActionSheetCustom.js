...
import {
  ...
  Modal as nativeModal, Animated, ScrollView, Easing
} from 'react-native'
...
class ActionSheet extends Component {
  constructor(props) {
    ...
    this.state = {
      visible: false,
      sheetPositionY: new Animated.Value(0),
    }
  }
  ...
  showSheet() {
    this.state.sheetPositionY.setValue(0);
  ...
  hideSheet(callback) {
    this.state.sheetPositionY.setValue(1);
    Animated.timing(this.state.sheetPositionY, {
      toValue: 0,
      duration: 300,
      useNativeDriver: true,
  ...
  render() {
    ...
    const Modal = customModal || nativeModal
    const yPos = sheetPositionY.interpolate({
            inputRange: [0, 1],
            outputRange: [this.translateY, 0]})
    const fade = sheetPositionY.interpolate({
            inputRange: [0, 1],
            outputRange: [0, 0.4]})
    return (
      <Modal
      ...
      >
        <View style={styles.wrapper}>
          <Animated.Text style={[styles.overlay,{opacity:fade}]} onPress={this.cancel} />
          <Animated.View
            style={[
              styles.backdrop,
              {
